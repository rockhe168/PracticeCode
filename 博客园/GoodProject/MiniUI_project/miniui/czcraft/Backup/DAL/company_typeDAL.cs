 ...
 */ 
{
	...
	public partial class company_typeDAL
	{
		...
		{
			string sql="insert into company_type(Companyid,Typeid) output inserted.id values(@Companyid,@Typeid)";
		...
		{
			string sql="update company_type set Companyid=@Companyid,Typeid=@Typeid where id=@id";
		...
		{
		return SqlHelper.ExecuteNonQuery("delete from company_type where id=@id",
		...
		{
		return SqlHelper.ExecuteNonQuery("delete from company_type where id in ("+strID+")");
		}
		/// <summary>
		/// 将DataRow转化为Model实体
		/// </summary>
		...
		/// <returns>执行状态</returns>
		private static company_type ToModel(DataRow row)
		{
			company_type model=new company_type();
		...
		{
			DataTable dt=SqlHelper.ExecuteDataTable("select * from company_type where id=@id",
(DbParameter)new SqlParameter("id",id));
			if(dt.Rows.Count>1){
			...
			company_type model=ToModel(row);
			return model;
		}
		/// <summary>
		/// 列出tableName所有的实体信息
		...
		{
			List<company_type> list=new List<company_type>();
			DataTable dt=SqlHelper.ExecuteDataTable("select * from company_type");
		...
		public IEnumerable<company_type> ListByPagination(string tableName,string InnerJoin,string strGetFields,string sortId,int PageSize,int PageIndex,string OrderType,string strWhere)
		{
			List<company_type> list=new List<company_type>();
		...
		public int GetCount(string strWhere)
		{
			...
			return SqlHelper.ExecuteSelectFirstNum("select count(1) from company_type"+strWhere);
		}
	}
}

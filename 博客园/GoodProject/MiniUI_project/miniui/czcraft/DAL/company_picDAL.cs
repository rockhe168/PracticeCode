 ...
 */ 
{
	...
	public partial class company_picDAL
	{
		...
		{
			string sql="insert into company_pic(Name,Picpath,Companyid) output inserted.Id values(@Name,@Picpath,@Companyid)";
		...
		{
			string sql="update company_pic set Name=@Name,Picpath=@Picpath,Companyid=@Companyid where id=@id";
		...
		{
		return SqlHelper.ExecuteNonQuery("delete from company_pic where id=@id",
		...
		{
		return SqlHelper.ExecuteNonQuery("delete from company_pic where id in ("+strID+")");
		}
		/// <summary>
		/// 将DataRow转化为Model实体
		/// </summary>
		...
		/// <returns>执行状态</returns>
		private static company_pic ToModel(DataRow row)
		{
			company_pic model=new company_pic();
		...
		{
			DataTable dt=SqlHelper.ExecuteDataTable("select * from company_pic where id=@id",
(DbParameter)new SqlParameter("id",id));
			if(dt.Rows.Count>1){
		...
		public IEnumerable<company_pic> ListAll()
		{
			...
			DataTable dt=SqlHelper.ExecuteDataTable("select * from company_pic");
		...
		public IEnumerable<company_pic> ListByPagination(string tableName,string InnerJoin,string strGetFields,string sortId,int PageSize,int PageIndex,string OrderType,string strWhere)
		{
			List<company_pic> list=new List<company_pic>();
		...
		public int GetCount(string strWhere)
		{
			...
			return SqlHelper.ExecuteSelectFirstNum("select count(1) from company_pic"+strWhere);
		}
	}
}

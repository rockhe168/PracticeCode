...
<div class="pageHeader">
    <form id="pageForm<%=PageId %>" action="<%=HtmlHelper.ForActionLink("list") %>" method="post" class="required-validate" >
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <li><a class="add" href="/AuthRole/Show.bee?id=-1" target="dialog"  mask="true" title="创建" rel="CD<%=PageId %>"><span>添加</span></a></li>
    ...
    <table id="table<%=PageId %>" class="table" width="1000" layouth="113">
        <thead>
        ...
        <%
            ...
            <tr>
                    ...
                    foreach (System.Data.DataColumn column in dataTable.Columns)
				{
				 ...
				 <td>
					<a title='删除' target='ajaxTodo' href='/AuthRole/Delete.bee?id=<%=row[0].ToString() %>' class='btnDel'>删除</a>					
					<a title='编辑' target="dialog"  mask="true" rel="CD<%=PageId %>" href='/AuthRole/Show.bee?id=<%=row[0].ToString() %>' class='btnEdit'>编辑</a>
					<a title='编辑权限--<%=row["Name"] %>' target="navTab"  mask="true" rel="CD<%=PageId %>" href='/AuthRole/ShowPermission.bee?id=<%=row[0].ToString() %>' class="button"><span>编辑权限</span></a>
					
<%--					<a title='查看人员--<%=row["Name"] %>' target="navTab"  mask="true" rel="CD<%=PageId %>" href='/AuthRole/ShowUser.bee?id=<%=row[0].ToString() %>' class="button"><span>查看人员</span></a>--%>
				</td>
			</tr>
			<%}} %>
        </tbody>
